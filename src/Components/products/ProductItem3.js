...
import Product from '../DummyData/Product3';
...
function ProductItem3({ cartItems, setCartItems }) {
  const navigate = useNavigate();
  const location = useLocation();

  const [showProgressBar, setShowProgressBar] = useState(false);
  const [products, setProducts] = useState(Product.products);
  const [quantities, setQuantities1] = useState(new Array(products.length).fill(1));
  ...
  const handleIncrease = (index) => {
    ...
    setQuantities1(quantities.map((q, i) => i === index ? q + 1 : q));
  }
  useEffect(() => {
    setQuantities1(new Array(products.length).fill(1));
  }, [location]);

  const handleDecrease = (index) => {
    console.log('Decrease clicked:', index);
    setQuantities1(quantities.map((q, i) => i === index && q > 1 ? q - 1 : q));
  }
  ...
  return (
    <>
      <NavigationBar cartItems={cartItems} showSlideshow={false} showHeader={false} showDropdown={false} onSearch={setSearchTerm} />

      <div className="container">
        <br></br>
        <h1 style={{ position: 'relative', top: '0px', marginBottom: '1rem' }} id="heading">Grocery</h1>
        <br></br>
        <br></br>
        <br></br>
        <br></br>
